        ...
        function analyzeTrendsAndRender(diagnoses, orders, activities) {
            ...
            document.getElementById('trend-total-assessments').textContent = diagnoses.length;
            document.getElementById('trend-high-risk').textContent = highRisk;
            ...
            if (monthCtx) {
                ...
                trendMonthlyChart = new Chart(monthCtx, {
                    ...
                    options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255,255,255,0.05)' } }, y: { ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255,255,255,0.05)' } } }, plugins: { legend: { labels: { color: '#ccc' } } } }
                });
            }
            ...
            if (riskCtxEl) {
                ...
                trendRiskChart = new Chart(riskCtxEl, {
                    ...
                    options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { color: '#9ca3af' }, grid: { display: false } }, y: { ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255,255,255,0.05)' } } }, plugins: { legend: { display: false } } }
                });
            }
            
            // --- MEDICINE DEMAND PREDICTION ---
            // Based on diagnosis types, predict what medicines will be needed
            ...
            // --- AI INSIGHTS ---
            const insights = generateAIInsights(diagnoses, typeCounts, riskCounts, growth, orders);
            ...
            // Use diagnoses as proxy for activity if no activity table data
            ...
            if (actCtx) {
                ...
                trendActivityChart = new Chart(actCtx, {
                    type: 'bar',
                    data: {
                        labels: Object.keys(activityByDay).map(d => new Date(d).toLocaleDateString('default',{weekday:'short',month:'short',day:'numeric'})),
                        datasets: [{
                            label: 'User Actions',
                    ...
                    options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { color: '#9ca3af', font: {size:10} }, grid: { display: false } }, y: { ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255,255,255,0.05)' } } }, plugins: { legend: { display: false } } }
                });
            }
            ...
            if (breakdown) {
                const totalActivity = Object.values(activityByDay).reduce((s,v) => s+v, 0);
                const typeOfActivity = {};
                activities.forEach(a => { typeOfActivity[a.activity_type] = (typeOfActivity[a.activity_type]||0)+1; });
                const breakdownItems = Object.entries(typeOfActivity).slice(0,5);
                breakdown.innerHTML = `
                    <div class="text-sm font-semibold text-gray-300 mb-2">Last 7 Days Activity</div>
                    ${breakdownItems.length ? breakdownItems.map(([k,v]) => `
                        ...
                        </div>`).join('') : `<div class="text-xs text-gray-400">Total activity events this week: <span class="text-blue-400 font-bold">${totalActivity}</span></div>`}
                `;
            }
    ...
    <script type="module" src="ballpit.js">